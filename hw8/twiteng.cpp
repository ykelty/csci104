...
void TwitEng::addTweet(const string& username, const DateTime& time, const string& text){
 ...
 void TwitEng::follow(const string& username1, const string& username2){
 	for(set<User*>::iterator itr = users.begin(); itr != users.end(); ++itr){
 		if((*itr)->name() == username1){
 			for(set<User*>::iterator it = users.begin(); it != users.end(); ++it){
 				if((*it)->name() == username2){
 					(*itr)->addFollowing(*it);
 					(*it)->addFollower(*itr);
 				}
 			}
 ...
 void TwitEng::trending(int n){
 	...
 	for(map<string, vector<Tweet*> >::iterator it = hashtags.begin(); it != hashtags.end(); ++it)
 	{

 		int count = 0;
 		count = it->second.size();
 		TagData aTag(it->first, count);
 		trending_.push(aTag);
